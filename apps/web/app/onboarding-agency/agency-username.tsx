...
import { UPDATE_AGENCY_ONBOARDING_USERNAME } from "../../lib/mutations";
import { IS_USERNAME_AVAILABLE } from "../../lib/queries";
import { getUsernameInputRules } from "../../lib/utils";

export default function AgencyUsername({
  defaultValues,
  nextStep,
}: {
  ...
  const [updateUsername, { loading }] = useAuthMutation(
    UPDATE_AGENCY_ONBOARDING_USERNAME,
  );
  ...
  const onSubmit: SubmitHandler<typeof defaultValues> = async (data) => {
    if (data.username) {
      ...
      if (res?.data?.addAgencyUsername) nextStep();
    }
  };
  return (
    <Form
